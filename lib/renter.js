...
Renter.prototype.getContact = function () {
  var self = this;
  // return network.contact if node is newed
  // or generate a contact with same logic
  return ( self.network ? self.network.contact : storj.Contact({
    address: self._opts.networkOpts.rpcAddress,
    port: self._opts.networkOpts.rpcPort,
    nodeID: storj.KeyPair(self._opts.networkPrivateKey).getNodeID()
  }));
};

Renter.prototype.addSeedList = function (seedContact) {
  if (seedContact.node !== this.getContact().nodeID) {
    // only add seed if they are not same node
    this._opts.networkOpts.seedList.push(
      storj.utils.getContactURL(seedContact)
    );
  }
};
...
Renter.prototype.start = function(callback) {
  ...
  this._loadKnownSeeds(function(err, seeds) {
    ...
    this.network = storj.RenterInterface({
      ...
      joinRetry: this._opts.networkOpts.joinRetry,
